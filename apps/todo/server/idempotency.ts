...
/**
 * A durable, cross-process execution ledger for the standalone app. SQLite's
 * write transaction elects one owner before the Notion request begins; a
 * renewable lease allows recovery when that process crashes mid-request.
 */
export function createSqliteIdempotencyStore(path: string): NotionIdempotencyStore {
  ...
  const acquire = (
    ...
    try {
      const existing = read.get(operation.scope, operation.key) as StoredOperation | undefined;
    ...
    async execute<T>(operation: NotionIdempotencyOperation, run: () => Promise<T>) {
      ...
      while (true) {
        ...
        const renewal = setInterval(() => {
          const now = Date.now();
          renew.run(now + leaseDurationMs, now, operation.scope, operation.key, owner);
        }, leaseDurationMs / 3);
        renewal.unref();
        try {
